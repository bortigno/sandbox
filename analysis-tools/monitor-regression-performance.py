...
phase_space_string = "muons.pt>0."
#phase_space_string = "(max(abs(muons.eta[0]),abs(muons.eta[1])) > 1.7)"
#phase_space_string = "(min(abs(muons.eta[0]),abs(muons.eta[1])) < 0.9)"

full_cut_string_and_label = ["{0}*({1}&{2})".format(weight_string,test_event_string,phase_space_string), "all_events"]
...
tree.Draw("(muons.pt-muons.GEN_pt)/muons.GEN_pt>>h_res_pf",full_cut_string_and_label[0])
tree.Draw("(muons_corr_regr*muons.pt - muons.GEN_pt)/muons.GEN_pt>>h_res_regr",full_cut_string_and_label[0])
...
c_res.SaveAs("{0}_{1}_res.pdf".format(input_id,full_cut_string_and_label[1]))
